...
Interactive runs ask about the card (use an image? custom title?) — but only for choices you haven't already made here or with a flag; set `title_card_image`/`title_text` once and the questions disappear:
